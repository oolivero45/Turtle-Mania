...
var skinWindow = function() {
  ...
  if (presence !== "skin") {
    ...
    menu.id = 'menu_skin';
    menu.className = 'menu';
    menu.innerHTML = '<h2 class="title">' + i18nGet("menu.skin") + '</h2><button class="menu_close" onclick="javascript:closeMenu();"><i class="fa fa-fw fa-lg fa-times"></i></button>';
...
var unlockedSkins = [["classic"],["classic"],["classic"],["none"]];
...
var updateTemperature = function(amount = 0) {
  temperature += amount;
  var removeClass = "temperaturehigh";
  var addClass = "temperaturelow";
  if (temperature < (safeTemperatureRange[0] - 40)) {
    if (inventoryGet("item.tank.fluid.thermogel") > 0) {
      inventoryRemove("item.tank.fluid.thermogel",1);
      temperature = 293;
    } else {
      removeHealth(10);
    }
  } else if (temperature > (safeTemperatureRange[1] + 40)) {
    if (inventoryGet("item.tank.fluid.coolant") > 0) {
      inventoryRemove("item.tank.fluid.coolant",1);
      temperature = 293;
    } else {
      removeHealth(10);
    }
  ...
  if (temperature <= 0) {
    temperature = 0;
    die();
  } else if (temperature >= 586) {
    temperature = 586;
    die();
  }
  $("#temperature").width(Math.floor((temperature / 586) * 200));
  $("#temperature").addClass(addClass);
  $("#temperature").removeClass(removeClass);
  return health;
}

var addTemperature = function(amount) {
  var result = updateTemperature(amount);
  return result;
}

var removeTemperature = function(amount) {
  amount = 0 - amount;
  var result = updateTemperature(amount);
  return result;
}

var equaliseTemperature = function(amount) {
  if (temperature === 293) {
    return true;
  }
  if (amount < 0) {
    if (temperature + amount < 293) {
      amount = 293 - temperature;
    }
    removeTemperature(Math.abs(amount));
    return true;
  } else {
    if (temperature + amount > 293) {
      amount = 293 - temperature;
    }
    addTemperature(amount);
    return true;
  }
}
...
if (seedTest === false) {
  var healthLoop = setInterval(function(){
    ...
    var safeTemp = false;
    var tempChange = 0;
    if (planets[currentPlanet].temperature >= safeTemperatureRange[0] && planets[currentPlanet].temperature <= safeTemperatureRange[1]) {
      safeTemp = true;
    } else {
    ...
    if (safeTemp === true) {
      ...
      equaliseTemperature(20);
    } else {
      $("#temperaturebar").show();
      if (tempChange > 0) {
        addTemperature(tempChange);
      } else {
        removeTemperature(Math.abs(tempChange));
      }
    }
    addHealth(1);
  },1000);
